...
RUN mkdir -p /tmp/protobuf ; \
	cd /tmp/protobuf ; \
	wget -c https://github.com/protocolbuffers/protobuf/releases/download/v3.17.2/protobuf-all-3.17.2.tar.gz -O - | tar -xz ; \
	protobuf-3.17.2/configure ; \
	cd protobuf-3.17.2 ; \
	make ; \
	make check ; \
	make install ; \
...
RUN mkdir -p /go/src/github.com/AsynkronIT/
WORKDIR /go/src/github.com/AsynkronIT
RUN	cd /go/src/github.com/AsynkronIT/ && \
	git clone https://github.com/tutumagi/protoactor-go.git
WORKDIR /go/src/github.com/AsynkronIT/protoactor-go
RUN	cd /go/src/github.com/AsynkronIT/protoactor-go && \
	go get ./... ; \
	make
